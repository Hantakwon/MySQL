...
-- Seller
ALTER TABLE Seller RENAME COLUMN compnay TO company;
...
-- Product
ALTER TABLE Product MODIFY prodName VARCHAR2(50);
INSERT INTO Product VALUES(100101, 11, 10003, '반팔티 L~2XL', 25000, 869, 132, 20);
INSERT INTO Product VALUES(100110, 10, 10004, '트레이닝 통바지', 38000, 1602, 398, 15);
INSERT INTO Product VALUES(110101, 10, 10003, '신상 여성운동화', 76000, 160, 40, 5);
INSERT INTO Product VALUES(120101, 12, 10010, '암소 1등급 구이셋트 1.2kg', 150000, NULL, 87, 15);
INSERT INTO Product VALUES(120103, 12, 10010, '바로구이 부채살 250g', 21000, NULL, 61, 10);
INSERT INTO Product VALUES(130101, 13, 10006, '[ANF] 식스프리 강아지 사료', 56000, 58, 142, 0);
INSERT INTO Product VALUES(130112, 13, 10006, '중대형 사계절 강아지옷', 15000, 120, 80, 0);
INSERT INTO Product VALUES(141001, 14, 10001, '라떼 2인 소파/방수 패브릭', 320000, NULL, 42, 0);
INSERT INTO Product VALUES(170115, 17, 10007, '지포스 3080 그래픽카드', 900000, 28, 12, 12);
INSERT INTO Product VALUES(160103, 16, 10009, '치닝디핑 33BR 철봉', 120000, 32, 28, 0);
...
INSERT INTO OrdersItem (orderNo, prodNo, itemPrice, itemDiscount, itemCount) VALUES ('22010210010', 110101, 79000, 5, 1);
...
INSERT INTO OrdersItem (orderNo, prodNo, itemPrice, itemDiscount, itemCount) VALUES ('22010710110', 100110, 120103, 10, 5);

-- Cart
INSERT INTO Cart (userId, prodNo, cartProdCount, cartProdDate) VALUES ('user1', 100101, 1, '2022-01-10 10:50:12');
INSERT INTO Cart (userId, prodNo, cartProdCount, cartProdDate) VALUES ('user1', 100110, 2, '2022-01-10 10:50:12');
INSERT INTO Cart (userId, prodNo, cartProdCount, cartProdDate) VALUES ('user3', 120103, 1, '2022-01-10 10:50:12');
...
SELECT p.prodNo, c.cateName, p.prodName, p.prodPrice, s.manager, s.tel
FROM Product p JOIN Category c ON p.cateNo = c.cateNo
               JOIN Seller s ON p.sellerNo = s.sellerNO;

-- 문제 3 : 모든 고객의 아이디, 이름, 휴대폰, 현재포인트, 적립포인트 총합을 조회하시오. 단 적립포인트 내역이 없으면 0으로 출력
SELECT u.userId, u.name, u.hp, u.point, NVL(SUM(p.point),0) 
FROM "User" u LEFT JOIN Point p ON u.userId = p.userId
GROUP BY u.userId, u.name, u.hp, u.point;

-- 문제 4 : 모든 주문의 주문번호, 주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오. 단 주문금액에 10만원 이상, 큰 금액순으로 조회, 금액이 같으면 이름이 사전순으로 될것
SELECT o.orderNo, u.userId, u.name, o.orderTotalPrice, o.orderDate
FROM "User" u JOIN Orders o ON u.userId = o.userId
...
FROM Orders o JOIN "User" u ON u.userId = o.userId
              JOIN OrdersItem i ON i.orderNo = o.orderNo
              JOIN Product p ON p.prodNo = i.prodNo
GROUP BY o.orderNo, u.userId, u.name, o.orderDate;

-- 문제 6 : 모든 상품의 상품번호, 상품명, 상품가격, 할인율, 할인된 가격을 조회하시오.
SELECT prodNo, prodName, prodPrice, prodDiscount, prodPrice - ((prodPrice / 100) * prodDiscount)
FROM Product;

-- 문제 7 : 고소영이 판매하는 모든 상품의 상품번호, 상품명, 상품가격, 재고수량, 판매자이름을 조회하시오. 
SELECT p.prodNo, p.prodName, p.prodPrice, p.prodStock, s.manager
FROM Product p JOIN Seller s ON p.sellerNo = s.sellerNo
WHERE s.manager = '고소영';

-- 문제 8 : 아직 상품을 판매하지 않은 판매자의 판매자번호, 판매자상호, 판매자 이름, 판매자 연락처를 조회
SELECT sellerNo, company, manager, tel FROM Seller 
WHERE sellerNo IN (SELECT sellerNo FROM Product
                   WHERE prodNO NOT IN (SELECT prodNo
                                        FROM Orders o JOIN OrdersItem i ON o.orderNo = i.orderNo));

-- 문제 9 : 모든 주문상세내역 중 개별 상품 가격과 개수 그리고 할인율이 적용된 가격을 구하고 그 가격으로 주문별 총합을 구해서 주문별 총합이 10만원이상 그리고 큰 금액 순으로 `주문번호`, `최종총합`을 조회하시오. 
SELECT i.orderNo, SUM(i.itemPrice * i.itemCount * (1 - i.itemDiscount / 100))
FROM OrdersItem i
GROUP BY i.orderNo
HAVING SUM(i.itemPrice * i.itemCount * (1 - i.itemDiscount / 100)) >= 100000
ORDER BY SUM(i.itemPrice * i.itemCount * (1 - i.itemDiscount / 100)) DESC;
  

-- 문제10 : 장보고 고객이 주문했던 모든 상품명을 `고객명`, `상품명`으로 조회하시오. 단 상품명은 중복 안됨, 상품명은 구분자 , 로 나열
SELECT u.name, LISTAGG(p.prodName, ', ')
FROM Orders o JOIN "User" u ON u.userId = o.userId
              JOIN OrdersItem i ON i.orderNo = o.orderNo
              JOIN Product p ON p.prodNo = i.prodNo
GROUP BY u.name
