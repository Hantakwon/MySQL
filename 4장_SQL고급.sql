...
-- 실습 4-16
SELECT sum(sale) FROM Sales WHERE uid = (SELECT uid FROM Member WHERE name = '김유신');

-- 실습 4-17
SELECT m.name, m.dep, m.pos, s.year, SUM(s.sale) FROM Member AS m JOIN Sales AS s ON m.uid = s.uid
